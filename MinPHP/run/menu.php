...
<?php } else{
    ...
    <button onclick="click1(1)">待开发</button>
    <button onclick="click1(2)">待对接</button>
    <button onclick="click1(3)">对接完毕</button>
    <button onclick="click1(4)">显示全部</button>
...
<script>
    var $COOKIE_KEY = "<?php echo C('cookie->navbar')?>"; //记录左侧菜单栏的开打与关闭状态的cookie的值
        ...
        //点击
        function click1(status) {
            $(".menu").hide();
            if (status < 4) {
                $(".menu").attr('status')
                for(let i=0;i<$(".menu").length;i++){
                    if($(".menu")[i].getAttribute('status')==status){
